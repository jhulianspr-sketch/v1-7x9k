...
// ===== MANEJO DE FORMULARIO DE CONTACTO CON FIREBASE =====
        ...
        if (contactForm) {
            contactForm.addEventListener('submit', async (e) => {
                ...
                if (!formData.name || !formData.email || !formData.message) {
                    showTerminalMessage('ERROR: Todos los campos son requeridos', 'error');
                    return;
                }
                
                // Mostrar estado de envío
                submitBtn.querySelector('.btn-text').textContent = '[TRANSMITIENDO...]';
                submitBtn.disabled = true;
                
                try {
                    // Importar Firebase dinámicamente
                    const { saveContactMessage } = await import('./firebase.js');
                    ...
                    if (result.success) {
                        ...
                        submitBtn.style.background = 'linear-gradient(135deg, #27c93f, #00ff41)';
                        
                        showTerminalMessage(`Mensaje transmitido exitosamente. ID: ${result.id}`, 'success');
                    ...
                    submitBtn.querySelector('.btn-text').textContent = '[ERROR DE TRANSMISIÓN]';
                    submitBtn.style.background = 'linear-gradient(135deg, #ff5f56, #ff0000)';
    ...
    console.log('%c🔥 Firebase conectado', 'color: #ff9800; font-size: 12px;');
